...
class Config:
    ...
    def __init__(self):
        ...
        self.risk_max_position_per_symbol = float(
            os.getenv("RISK_MAX_POSITION_PER_SYMBOL", "0.05")
        )
        self.risk_max_total_exposure = float(
            os.getenv("RISK_MAX_TOTAL_EXPOSURE", "0.8")
        )
        self.risk_max_single_trade_size = float(
            os.getenv("RISK_MAX_SINGLE_TRADE_SIZE", "0.02")
        )
        self.risk_default_stop_loss_pct = float(
            os.getenv("RISK_DEFAULT_STOP_LOSS_PCT", "0.02")
        )
        self.risk_default_take_profit_pct = float(
            os.getenv("RISK_DEFAULT_TAKE_PROFIT_PCT", "0.06")
        )
        self.risk_trailing_stop_pct = float(os.getenv("RISK_TRAILING_STOP_PCT", "0.01"))
        self.risk_max_daily_drawdown = float(
            os.getenv("RISK_MAX_DAILY_DRAWDOWN", "0.05")
        )
        self.risk_max_total_drawdown = float(
            os.getenv("RISK_MAX_TOTAL_DRAWDOWN", "0.15")
        )
        self.risk_volatility_threshold = float(
            os.getenv("RISK_VOLATILITY_THRESHOLD", "0.05")
        )
        ...
        self.trading_default_quantity_eth = float(
            os.getenv("TRADING_DEFAULT_QUANTITY_ETH", "0.01")
        )
        self.trading_default_quantity_usdt = float(
            os.getenv("TRADING_DEFAULT_QUANTITY_USDT", "10.0")
        )
        self.trading_min_order_size_btc = float(
            os.getenv("TRADING_MIN_ORDER_SIZE_BTC", "0.000001")
        )
        self.trading_min_order_size_eth = float(
            os.getenv("TRADING_MIN_ORDER_SIZE_ETH", "0.00001")
        )
        ...
        self.signal_macd_signal_window = int(
            os.getenv("SIGNAL_MACD_SIGNAL_WINDOW", "9")
        )

        # Monitoring Configuration
        self.monitoring_error_rate_threshold = float(
            os.getenv("MONITORING_ERROR_RATE_THRESHOLD", "0.1")
        )
        self.monitoring_api_error_rate_threshold = float(
            os.getenv("MONITORING_API_ERROR_RATE_THRESHOLD", "0.05")
        )

        # Portfolio Configuration
        self.portfolio_initial_value = float(
            os.getenv("PORTFOLIO_INITIAL_VALUE", "100000.0")
        )
        ...
        self.symbol_risk_overrides = {
            "BTCUSDT": {
                ...
                "volatility_multiplier": float(
                    os.getenv("BTC_VOLATILITY_MULTIPLIER", "1.0")
                ),
            },
            "ETHUSDT": {
                "max_position": float(os.getenv("ETH_MAX_POSITION", "0.08")),
                "volatility_multiplier": float(
                    os.getenv("ETH_VOLATILITY_MULTIPLIER", "1.2")
                ),
            },
        }
    ...
    def validate(self):
        ...
        if self.binance_testnet:
            if not self.binance_api_key or not self.binance_api_secret:
                print(
                    "[ERROR] Testnet mode requires BINANCE_API_KEY and BINANCE_API_SECRET to be set in the environment."
                )
        ...
        if not self.demo_mode and (
            ...
            print(
                "[ERROR] Live trading mode requires BINANCE_API_KEY and BINANCE_API_SECRET."
            )
            raise SystemExit(1)

    def get_symbol_risk_config(self, symbol: str) -> Dict[str, Any]:
        """Get risk configuration for a specific symbol"""
        return self.symbol_risk_overrides.get(
            symbol, {"max_position": 0.05, "volatility_multiplier": 1.0}
        )
