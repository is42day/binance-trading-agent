...
"""
        ...
        try:
            ...
            if risk_result["approved"]:
                ...
                self.portfolio.add_trade(
                    trade_id=result.get(
                        "order_id",
                        f"cli_sell_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    ),
                    symbol=symbol,
                    side="SELL",
                    quantity=quantity,
    ...
    def do_status(self, line):
        ...
        try:
            ...
            print(
                f"Emergency Stop: {emergency_emoji} {'ACTIVE' if risk_status['emergency_stop'] else 'INACTIVE'}"
            )
