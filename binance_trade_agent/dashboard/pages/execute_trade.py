...
@callback(Output("trade-price-input", "disabled"), Input("trade-order-type", "value"))
...
def place_trade_order(n_clicks, symbol, side, order_type, quantity, price):
    ...
    try:
        ...
        if not risk_check.get("approved", False):
            ...
            return (
                dbc.Alert(f"⚠️ Risk Check Failed: {reason}", color="warning"),
                None,
                None,
            )
        ...
        if order_result.get("success"):
            ...
            return (
                dbc.Alert(f"❌ Order Failed: {error_msg}", color="danger"),
                None,
                None,
            )
