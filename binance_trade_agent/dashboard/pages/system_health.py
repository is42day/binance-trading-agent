        ...
        metrics = dbc.Row(
            [
                dbc.Col(
                    [
                        create_metric_card(
                            label="Uptime", value=uptime_str, icon="⏱️", status="success"
                        )
                    ],
                    lg=3,
                    md=6,
...
def update_system_status(n_intervals):
    ...
    try:
        ...
        status_items = [
            ...
            (
                "Demo Mode",
                "ON" if demo_mode else "OFF",
                "#27ae60" if demo_mode else "#e74c3c",
            ),
            (
                "Binance Testnet",
                "ON" if binance_testnet else "OFF",
                "#27ae60" if binance_testnet else "#e74c3c",
            ),
            (
                "Production Ready",
                "YES" if production_ready else "NO",
                "#27ae60" if production_ready else "#e74c3c",
            ),
        ]
        ...
        status_display.append(
            html.Div(
                [
                    ...
                    html.Span(
                        last_updated[:19] if last_updated else "N/A",
                        style={"color": "#ff914d", "fontSize": "0.75rem"},
                    ),
                ],
                style={
                    "marginTop": "1rem",
...
def update_api_status(n_intervals):
    ...
    try:
        ...
        api_status = (
            ...
            else "Degraded" if api_error_rate < 50 else "Disconnected"
        )
        ...
        status_color = (
            ...
            else "#f39c12" if api_status == "Degraded" else "#e74c3c"
        )
        ...
        status_display = [
            dbc.Alert(
                [
                    ...
                    html.P(
                        f"Error Rate: {api_error_rate:.2f}%",
                        style={
                            "marginBottom": "0",
                            "fontSize": "0.875rem",
                            "color": status_color,
                ...
                color=(
                    "success"
                    if api_status == "Connected"
                    else "warning" if api_status == "Degraded" else "danger"
                ),
                style={"marginBottom": "1rem"},
            ),
            dbc.Alert(
                [
                    ...
                    html.P(
                        f"Error Rate: {trade_error_rate:.2f}%",
                        style={
                            "marginBottom": "0",
                            "fontSize": "0.875rem",
                            "color": trade_color,
