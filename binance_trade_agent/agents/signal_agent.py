...
class SignalAgent:
    ...
    def __init__(
        ...
        self.test_mode = test_mode or bool(
            os.environ.get("SIGNAL_AGENT_TEST_MODE", "").lower() in ("1", "true", "yes")
        )
        ...
        if strategy_parameters and strategy_name:
            ...
            if self.strategy_manager.create_strategy(
                strategy_type, custom_name, strategy_parameters
            ):
    ...
    def create_custom_strategy(
        self, name: str, strategy_type: str, parameters: Dict[str, Any]
    ) -> bool:
        """
    ...
    def compute_signal(self, ohlcv, indicator="rsi"):
        ...
        """
        self.logger.warning(
            "compute_signal() is deprecated. Use generate_signal() for enhanced strategy support."
        )

        if not ohlcv or not isinstance(ohlcv, list):
            return {"signal": "HOLD", "error": "OHLCV data must be a non-empty list."}
...
if __name__ == "__main__":
    ...
    if success:
        ...
        result = agent.generate_signal("BTCUSDT")
        print(
            f"   Custom strategy signal: {result['signal'].upper()} (confidence: {result['confidence']:.1%})"
        )
    else:
