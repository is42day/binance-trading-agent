...
class CombinedStrategy(BaseStrategy):
    ...
    def _combine_signals(
        self, rsi_result: StrategyResult, macd_result: StrategyResult
        ...
        if require_direction_agreement:
            if (
                rsi_result.signal != macd_result.signal
                and rsi_result.signal != SignalType.HOLD
                and macd_result.signal != SignalType.HOLD
            ):
    ...
    def _calculate_combined_levels(
        self,
        current_price: float,
        signal: SignalType,
        rsi_result: StrategyResult,
        macd_result: StrategyResult,
    ) -> Tuple:
    ...
    def get_risk_metrics(self, market_data: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        if len(market_data) >= self.requires_minimum_data():
            try:
                ...
                combined_risk = (
                    rsi_metrics.get("risk_level", 0.5) * rsi_weight
