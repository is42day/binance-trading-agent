...
class TradingOrchestrator:
    ...
    async def _generate_signal(
        self, symbol: str, correlation_id: str, strategy_name: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def _execute_trade(
        self, trade_decision: TradeDecision, correlation_id: str
    ...
    async def backtest_strategy(
        self, strategy_name: str, symbol: str, historical_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
...
async def demo_orchestration():
        ...
        if "error" not in analysis:
            ...
            if "strategy_comparison" in analysis:
                ...
                if "consensus" in comparison:
                    ...
                    print(
                        f"   Strategy Consensus: {consensus['signal']} (strength: {consensus['strength']:.1%})"
                    )
    ...
    try:
        decision = await orchestrator.execute_trading_workflow(
            symbol="BTCUSDT", quantity=config.get_default_quantity("BTCUSDT")
        )
    ...
    try:
        ...
        if "error" not in comparison:
            print(
                f"   Consensus: {comparison['consensus']['signal']} "
