...
def test_reconciliation_books_missing_filled_trade(monkeypatch):
...
def test_reconciliation_picks_up_filled_order_without_local_trade(monkeypatch):
    with tempfile.TemporaryDirectory() as _tmpdir:
        tmp_path = Path(_tmpdir)
        monkeypatch.setenv("DB_PATH", str(tmp_path / "portfolio.db"))
        monkeypatch.delenv("DATABASE_URL", raising=False)
...
def test_reconciliation_does_not_book_duplicate_trade(monkeypatch):
    with tempfile.TemporaryDirectory() as _tmpdir:
        tmp_path = Path(_tmpdir)
        monkeypatch.setenv("DB_PATH", str(tmp_path / "portfolio.db"))
        monkeypatch.delenv("DATABASE_URL", raising=False)
